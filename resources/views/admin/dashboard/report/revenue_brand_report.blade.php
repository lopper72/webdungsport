...
@section('content')
    @php
        $id = 5;
        $report_name = "Báo cáo doanh thu theo hãng";
    @endphp
    <div class="container mx-auto px-2 py-8 sm:px-6 md:px-8">
        ...
        <nav class="text-sm font-medium text-gray-500 py-4" aria-label="breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li class="flex items-center">
                    <a href="{{ route('admin.reports.prelim', ['id' => 5]) }}" class="text-blue-500 hover:text-blue-700">
                        {{ $report_name }}
                    </a>
                    &nbsp;/&nbsp;
                </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="px-4 py-6 md:px-6 xl:px-7.5">
                <div class="flex justify-between items-center">
                    <h4 class="text-xl font-bold text-black dark:text-white inline">
                        BÁO CÁO - <span class="uppercase font-bold text-sky-400">{{ $report_name }}</span>
                    </h4>
                </div>
            </div>

            <div class="overflow-x-auto px-4 py-6 md:px-6 xl:px-7.5">
                @livewire('admin.report.revenue-brand-report', ['id' => $id])
            </div>
        </div>
    </div>
@endsection
