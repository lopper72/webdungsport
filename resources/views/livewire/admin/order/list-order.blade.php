        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-sm">
                ...
                @foreach ($orders as $index => $order)
                    @php
                        $returnDetails = $order->salesReturnDetails;
                        $returnQuantity = $returnDetails->sum('quantity');
                        $returnAmount = $returnDetails->sum('total_amount');
                        $salesReturns = $returnDetails
                            ->pluck('salesReturn')
                            ->filter()
                            ->unique('id')
                            ->values();
                        $hasReturn = $salesReturns->isNotEmpty();
                    @endphp
                    <tr>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <input type="checkbox" name="cbx_delete_order" {{ $hasReturn ? 'disabled' : '' }} class="cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed" wire:model="selected_index.{{ $index }}" value="{{ $order->id }}">
                        </td>
                                ...
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    <div class="flex flex-wrap justify-center gap-1">
                                        @foreach ($salesReturns as $salesReturn)
                                            <a href="{{ route('admin.sales-returns.view', $salesReturn->id) }}" class="text-xs font-medium text-blue-600 hover:text-blue-800 hover:underline">
                                                {{ $salesReturn->code }}
                                            </a>
                                        @endforeach
                                    </div>
