                ...
                @foreach ($orders as $index => $order)
                    <tr>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <input type="checkbox" name="cbx_delete_order" class="cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" wire:model="selected_index.{{ $index }}" value="{{ $order->id }}">
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">{{ $orders->perPage() * ($orders->currentPage() - 1) + $loop->iteration }}</td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.orders.view', $order->id) }}" class="text-indigo-600 hover:text-indigo-900">{{ $order->code }}</a>
                                <button type="button" class="text-indigo-600 hover:text-indigo-900" onclick="copyToClipboard('{{ $order->code }}')">
                        ...
                        <td class="px-2 py-2 whitespace-nowrap text-left">{{ optional($order->customer)->name }}</td>
                        <td class="px-2 py-2 whitespace-nowrap text-left">{{ $order->created_at ? $order->created_at->timezone(config('app.timezone'))->format('d/m/Y H:i') : '-' }}</td>
                                                <td class="px-2 py-2 whitespace-nowrap text-center">
