...
@php
    $productOptions = $products
        ->map(fn ($product) => [
            ...
            'name' => trim(($product->code ? $product->code . ' - ' : '') . $product->name),
        ])
        ->values();
@endphp
...
<div
    ...
    x-data="{
        ...
        choose(product) {
            ...
            this.$wire.set('product_id', product.id).then(() => this.$wire.call('productChanged'));
        },
        chooseActive() {
        ...
        clear() {
            ...
            this.$wire.call('clearProductFilter');
        },
    }"
...
>
    <div class="relative">
        <input
            ...
            class="block w-full rounded-md border-0 py-1.5 pl-2 pr-9 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
            placeholder="Gõ mã hàng hoặc tên hàng"
            autocomplete="off"
        >
        <button
            type="button"
            ...
            x-cloak
            @click="clear()"
            class="absolute inset-y-0 right-0 flex items-center px-2 text-gray-400 hover:text-gray-700"
    ...
    >
        ...
        @foreach($products as $product)
            <option value="{{ $product->id }}">{{ trim(($product->code ? $product->code . ' - ' : '') . $product->name) }}</option>
        @endforeach
    </select>
