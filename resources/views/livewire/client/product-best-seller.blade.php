<div class="w-full bg-white py-4 md:py-6 px-2 md:px-6">
    <h2 class="text-lg lg:text-2xl font-bold text-gray-800 mb-2">Sản phẩm bán chạy</h2>
                ...
                <a href="{{route('product-detail', ['id' => $product->product->id, 'slug' => $product->product->slug])}}">
                    <div class="w-full aspect-w-1 aspect-h-1 overflow-hidden">
                        @if (count($product->product->productDetails) > 0 && $product->product->productDetails[0] && $product->product->productDetails[0]->image)
                            ...
                            @endphp
                            <img src="{{ asset('storage/images/products/' . $imageThumbnail) }}" alt="Hình ảnh sản phẩm" class="hover:grow hover:shadow-lg w-full h-full object-cover lazyload">
                        @else
                            <img src="{{ asset('library/images/image-not-found.jpg') }}" alt="Không có hình ảnh sản phẩm" class="hover:grow hover:shadow-lg  w-full h-full object-cover lazyload">
                        @endif
                    </div>
